 ...
 *
 * El nodo choclo (término acuñado por el equipo de desarrollo) es un nodo que
 * sólo sirve para marcar el inicio de una lista (no tiene datos y apunta a
 * NULL)
 *
 ...
 *
 * Itera sobre la lista hasta encontrar una coincidencia. Si el tipo de búsqueda
 * es por ci (tipo == true), retorono cuando esta coincide. Si es por nombre y
 * apellido (tipo == false), entonces retorno si hay coincidencia de ambos
 * miembros.
 *
 * @param head  Puntero al primer elemento de la lista
 * @param tipo  Define el tipo de búsqueda (true si es por ci, false si es por
 * nombre y apellido)
 * @param ci    CI del estudiante (se usa cuando tipo == true)
 * @param nombre    String con nombre del estudiante (se usa cuando tipo ==
 * false)
 ...
 *
 * Esta función asegura que el nodo en la posición `i` cumpla la propiedad de
 * heap: el valor en `arr[i]` debe ser mayor (según el comparador) que sus
 * hijos. Si no se cumple, intercambia el elemento con el mayor de sus hijos y
 * continúa el proceso recursivamente.
 *
...
/**
 * @brief Ordena la lista enlazada de estudiantes según un criterio de
 * comparación.
 *
 * Esta función toma la lista enlazada de estudiantes y la ordena utilizando
 * un array auxiliar de punteros a estudiante_t. Primero recorre la lista y
 * copia en el array los punteros a los estudiantes almacenados en cada nodo.
 * Luego aplica Heap Sort sobre ese array, usando la función comparadora
 * recibida como parámetro para definir el criterio de orden. Finalmente,
 * recorre nuevamente la lista y reasigna los punteros a estudiante de cada nodo
 ...
 * @param cmp Puntero a función comparadora que define el criterio de orden.
 *            Debe retornar:
 *              - un valor mayor que 0 si el primer estudiante es mayor que el
 * segundo,
 *              - 0 si ambos son equivalentes,
 *              - un valor menor que 0 si el primero es menor que el segundo.
 *
 * @note Si la lista está vacía o tiene un solo elemento, la función no realiza
 * cambios.
...
void sort_by_apellido(node_t *head) { sort_list(head, compare_by_apellido); }
